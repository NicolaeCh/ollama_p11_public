"""Streamlit UI for Ollama testing on IBM Power."""
...
CONFIG = load_config()
API_HOST = CONFIG.get("api_host") or "127.0.0.1"
OLLAMA_BASE_URL = f"{CONFIG.get('api_scheme', 'http')}://{API_HOST}:{CONFIG['container_port']}/api"
...
def calculate_metrics(final: Dict[str, Any], started: float, first_token_at: Optional[float], finished: float) -> Dict[str, Any]:
    eval_count = final.get("eval_count", 0) or 0
    eval_duration = final.get("eval_duration", 0) or 0
...
def chat_stream(
    ...
    with requests.post(f"{OLLAMA_BASE_URL}/chat", json=payload, stream=True, timeout=(10, 1800)) as response:
        response.raise_for_status()
...
def chat_once(model: str, messages: List[Dict[str, str]], temperature: float, num_predict: int, num_ctx: int, keep_alive: str) -> Dict[str, Any]:
    ...
    metrics = calculate_metrics(result, started, None, started + elapsed)
    metrics["content"] = result.get("message", {}).get("content", "")
    return metrics
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.caption(f"Ollama bind: {CONFIG.get('ollama_bind_host', CONFIG.get('host_bind', '0.0.0.0'))}:{CONFIG['container_port']}")
        ...
        num_predict = st.slider(
            ...
            32,
            MAX_TOKENS,
            min(int(CONFIG["default_num_predict"]), MAX_TOKENS),
            32,
        )
        num_ctx = st.slider(
            ...
            if prompt:
                ...
                with st.chat_message("assistant"):
                    try:
                        ...
                        st.caption(
                            f"TTFT: {stats.get('ttft', 0.0):.2f}s | TPS: {stats.get('tps', 0.0):.1f} | "
                            f"Total: {stats.get('total_time', 0.0):.2f}s | "
                        ...
                        st.session_state.perf_log.append(
                            {
                                ...
                                "ttft_s": stats.get("ttft", 0.0),
                                "tps": stats.get("tps", 0.0),
                                "total_s": stats.get("total_time", 0.0),
    ...
    with tab3:
        ...
        if perf_df.empty:
            ...
            with st.expander("Latest performance sample"):
